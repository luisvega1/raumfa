...
"container";delete $main.id;for(var d in window.countdowns)window.clearInterval(window.countdowns[d]);d=document.querySelectorAll(".material-tooltip");if(d.length)for(var c in d){if(c==d.length-1)break;d[c].parentElement.removeChild(d[c])}document.title="Raumfa";window.scrollTo(0,0);document.querySelector(".sidenav").style.transform="translateX(-105%)";document.querySelector(".sidenav-overlay")&&(document.querySelector(".sidenav-overlay").style.display="none");QueryString(function(a){window.$query=
...
function home(){var d=0;$main.innerHTML='<ul class="tabs"><li class="tab"><a href="#future" target="_self" class="active">Más Lanzamientos</a></li></ul>';$upcoming=document.createElement("div");$upcoming.style="padding: 0 !important";$main.appendChild($upcoming);$successful=document.createElement("div");$successful.style="padding: 0 !important";$main.appendChild($successful);$featured=document.createElement("div");$featured.id="featured";$featured.style="padding: 0 !important";$main.appendChild($featured);
...
c.launches[a];1!=e.statuscode&&6!=e.statuscode||1==e.tbdtime||new Countdown(e.net,"countdown"+e.id);$info.innerHTML='<div id="video"></div><div class="card-content"><h1><a class="tooltipped" data-tooltip="More Info" href="/#rocket='+e.rocket.id+'">'+e.name.replace("|","</a>|")+'</h1><h3 id="countdown'+e.id+'">'+e.status+'</h3><div id="chips"><a class="chip" onclick="refreshHome()"><i class="fas fa-sync"></i>Refresh</a><div class="chip tooltipped><img src="'+e.agency.icon+'?size=32" onerror=this.onerror=null;this.src="'+e.agency.countryFlag+'">'+e.agency.name+'</div><div class="chip tooltipped"><i class="far fa-compass"></i>'+e.location.pads[0].name+'</div><div class="chip tooltipped" id="launchdate"><i class="far fa-clock"></i>'+ReadableDateString(e.net)+'</div></div><p class="flow-text">'+e.description+'</p></div><div id="card-action" class="card-action"><a class="waves-effect waves-light btn hoverable" href="/#id='+
...
'" target="_blank"><i class="fas fa-external-link-square-alt"></i></a></div></div>');if(d==c.count-1)break;$upcoming.innerHTML='<div class="col s12 l'+Math.floor(12/(c.count-1))+'"><div class="card"><div class="card-content"><h5 class="header black-text truncate"><div style="color:#dd4d2e" class="tooltipped">'+e.name.replace(" | ","</div></h5><h4 class='header black-text truncate'>").split(" (")[0]+'</h4><div class="chip tooltipped"><img src="'+e.agency.icon+'?size=32" onerror=this.src="'+e.agency.countryFlag+'">'+e.agency.shortname+'</div><div class="chip tooltipped"><i class="fas fa-map-marker-alt"></i>'+e.location.name.split(",")[0]+'</div></br><div class="chip tooltipped"><i class="far fa-clock"></i>'+ReadableDateString(e.net)+'</div><h5 id="countdown'+e.id+'">'+e.status+'</h5></div><div class="card-action">< class="waves-effect waves-light btn hoverable" href="/#id='+
e.id+'">Detalles</button></div></div></div>'+$upcoming.innerHTML;d++}else $main.className="valign-wrapper",$main.innerHTML='<div class="container"><h1 class="white-text">'+(c.msg||"Error has occured")+"</h1></div>"});load("launch?limit=4&sort=desc&mode=summary&status=3,4,7",function(c){$successful.innerHTML='<ul class="tabs"><li class="tab"><a href="#history" target="_self" class="active">Lanzamientos Recientes</a></li></ul>';for(var a in c.launches){var e=c.launches[a];days=Math.floor((new Date-new Date(e.net))/
864E5);$successful.innerHTML+='<div class="col s12 m6 l3"><div class="card"><div class="card-content"><h5 class="header black-text truncate"><a style="color:#dd4d2e" class="tooltipped" data-tooltip="More info" href="/#rocket='+e.name.split(" |")[0].split("/")[0]+'">'+e.name.replace(" | ","</a></h5><h4 class='header black-text truncate'>").split(" (")[0]+'</h4><a class="chip tooltipped" data-tooltip="'+e.net+'"><i class="far fa-clock"></i>'+ReadableDateString(e.net)+'</a><a class="chip">'+(0<days?days + " Días del lanzamiento ":"Hoy")+"</a><h5>"+e.status+'</h5></div><div class="card-action"><a class="waves-effect waves-light btn hoverable" href="/#id='+e.id+'">Watch</a></div></div>'}for(a=1;a<=$successful.childElementCount-1;a++)$successful.children[a].className="col s12 l"+Math.floor(12/($successful.childElementCount-1))});getJSON(location.origin+"/data/sources.json",function(c){if(c.featuring){$featured.innerHTML='<ul class="tabs"><li class="tab"><a class="active">Featured</a></li></ul>';for(var a in c.featuring)$featured.innerHTML+=
...
function timeline(d,c){c=parseInt(c)||1;$main.innerHTML="";load("launch?mode=verbose&limit=20"+(d?"&sort=desc&status=3,4,7":"&status=1,2,5,6")+"&offset="+20*(c-1),function(a){for(var e in a.launches){var b=a.launches[e];$main.innerHTML+='<div class="col s12 m6"><div class="card"><div class="card-content"><h5 class="header black-text truncate"><a style="color:#dd4d2e" class="tooltipped" data-tooltip="More info" href="/#rocket='+b.name.split(" |")[0].split("/")[0]+'">'+b.name.replace(" | ","</a></h5><h4 class='header black-text truncate'>")+
'</h4><a class="chip tooltipped" data-tooltip="'+b.agency.name+'" href="/#agency='+b.agency.id+'"><img src="'+b.agency.icon+'?size=32" onerror=this.src="'+b.agency.countryFlag+'">'+b.agency.shortname+'</a><a class="chip tooltipped" data-tooltip="'+b.location.name+'" href="/#pad='+b.location.pads[0].id+'"><i class="fas fa-map-marker-alt"></i>'+b.location.name.split(",")[0]+'</a></br><a class="chip tooltipped" data-tooltip="'+b.net+'"><i class="far fa-clock"></i>'+ReadableDateString(b.net)+'</a><h5 id="countdown'+
...
function Countdown(d,c){padnumber=function(a,c){for(var b=a+"";b.length<c;)b="0"+b;return b};count=function(a){var c,b=Date.parse(a),d=new Date(ServerDate.now());a=Math.floor((b-d)/1E3);var h="Faltan - ";0>=a&&(h="L+ ",a=Math.floor((d-b)/1E3));var b=Math.floor(a/60),d=Math.floor(b/60),f=Math.floor(d/24),m=padnumber(a%60,2);60>a&&(c=h+m);m=padnumber(b%60,2)+":"+m;60>b&&(c=h+m);m=padnumber(d%24,2)+":"+m;24>d&&(c=h+m);1<f?c=h+f+" días "+m:1==f&&(c=h+f+" dia "+m);return c};c&&(countdown=setInterval(function(){(document.getElementById(c)||
